(function( $ ) {
	...
	wp.customize.bind( 'ready', function() {
			...
			if(numRepeaterItems > 0) {
				...
				if(numRepeaterItems > 1) {
					...
					for (i = 1; i < numRepeaterItems; ++i) {
						cnzAppendRow($(this), defaultValuesArray[i]);
					}
				}
			}
		});
		...
		$('.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
			...
			if(numItems > 1) {
				$(this).parent().slideUp('fast', function() {
					...
					$(this).remove();
					cnzGetAllInputs(parentContainer);
				})
			}
			else {
				$(this).parent().find('.repeater-input').val('');
				cnzGetAllInputs($(this).parent().parent().parent());
			}
		});
		...
		function cnzAppendRow($element, defaultValue = '') {
		...
		function cnzGetAllInputs($element) {
